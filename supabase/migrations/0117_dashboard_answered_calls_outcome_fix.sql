-- Fix dashboard „Atsiliepė“ / trend answered counts.
-- Root cause: RPC counted answered only via legacy Kanban column names on `call_status`
-- (Laukti, Siųsti laišką, …). New Kanban moves store outcomes as `answered` / `not_answered`,
-- which fell into `else → Skambinti` and were excluded from answeredCalls.
-- Align with app `isCallAnsweredByStatus` / `isCallNotAnsweredByStatus`.
