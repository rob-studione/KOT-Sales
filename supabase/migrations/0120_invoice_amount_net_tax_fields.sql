-- Invoice net amounts for UI/KPI without wrongly stripping 21% from 0% VAT rows.
-- Neksar: subtotal → amount_net, taxAmount → tax_amount, taxRate → tax_rate.
...
comment on column public.invoices.amount_net is 'Suma be PVM (Neksar subtotal).';
comment on column public.invoices.tax_amount is 'PVM suma (Neksar taxAmount).';
comment on column public.invoices.tax_rate is 'PVM tarifas % (Neksar taxRate), 0 = 0%/neapmokestinta.';
...
-- ------------------------------------------------------------
-- KPI RPC: vat_invoices_kpis (sum be PVM)
-- ------------------------------------------------------------
...
as $$
  select
    count(*)::bigint,
    coalesce(sum(public.invoice_amount_net(i.amount, i.amount_net, i.tax_amount, i.tax_rate)), 0)::numeric
...
grant execute on function public.vat_invoices_kpis() to authenticated;
...
acts_flags as (
  select
    au.*,
    case
      -- Explicit call outcomes (not Kanban columns)
      when au.call_status_lc in ('answered', 'atsiliepė', 'atsiliepe', 'not_answered', 'neatsiliepė', 'neatsiliepe')
        then null
      when au.call_status_raw in ('', 'Neatsiliepė') then 'Skambinti'
      when au.call_status_raw = 'Perskambins' then 'Perskambinti'
      -- Legacy labels → same as app LEGACY_CALL_STATUS_TO_KANBAN
      when au.call_status_raw in ('Laukti', 'Susisiekti vėliau', 'Aktualu pagal poreikį') then 'Perskambinti'
      when au.call_status_raw in ('Skambinti','Perskambinti','Siųsti laišką','Siųsti komercinį','Skubus veiksmas','Užbaigta')
...
trend_agg as (
  select
    local_day as day,
    count(*) filter (where action_type = 'call') as calls,
    count(*) filter (where action_type = 'call' and is_answered) as answered,
    count(*) filter (where action_type = 'call' and is_not_answered) as not_answered
  from acts_range
  group by 1
),
trend_full as (
...
-- Client list revenue: sum net amounts (0% VAT stays intact).
create or replace view public.v_client_list_from_invoices
...
with agg as (
  select
    ...
    sum(public.invoice_amount_net(i.amount, i.amount_net, i.tax_amount, i.tax_rate)) as total_revenue
  from public.invoices i
  group by 1
),
latest as (
...
grant select on public.v_client_list_from_invoices to authenticated;
