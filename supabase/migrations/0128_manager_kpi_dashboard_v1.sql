-- Manager KPI: aggregate activities in SQL (no 80k row dump to Node).
-- Attribution matches lib/crm/managerKpiDashboard.ts:
--   calls: action_type=call + non-null performed_by ∈ is_kpi_tracked users
--   commercial: coalesce(performed_by, work_item.assigned_to) ∈ tracked users
-- Answered flags mirror 0125 / 0127.
...
comment on function public.manager_kpi_dashboard_v1(date, date, date, date) is
  'Manager KPI dashboard aggregates (per tracked user + daily chart + optional compare).';
