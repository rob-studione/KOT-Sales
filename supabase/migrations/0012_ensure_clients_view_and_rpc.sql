-- Non-destructive: for databases that have public.invoices but never received 0006/0009
-- (e.g. synced data without running the full migration chain).
-- Creates/replaces v_client_list_from_invoices, recent_invoices_for_clients, dashboard_stats_from_invoices,
-- nullable company_code, and handle_new_invoice (skip companies insert when no code).
--
-- Prereq: public.invoices (and public.companies for the insert trigger — same as 0005/0009).
-- Does NOT run 0005 (that migration DROP-recreates tables — do not use on a populated DB).

do $$
begin
  if to_regclass('public.invoices') is null then
    raise exception '0012 requires public.invoices; apply earlier migrations first.';
  end if;
end $$;

-- ---------------------------------------------------------------------------
-- Match 0009: nullable company_code + UNKNOWN cleanup
-- ---------------------------------------------------------------------------
...
begin
  if exists (
    ...
    alter table public.invoices alter column company_code drop not null;
  end if;
end $$;

-- ---------------------------------------------------------------------------
-- Trigger: skip companies aggregate when no company code (0009)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Client list view + RPCs (same as 0009)
-- ---------------------------------------------------------------------------
