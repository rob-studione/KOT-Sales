-- Blokuoti kandidatus, jei ta pati įmonė jau aktyviame darbe
-- ne tik pagal exact client_key, bet ir pagal company_code / vat_code /
-- client_id (po vat→company_code backfill senos work eilutės lieka su client_id).
...
as $$
  ...
  active_work as (
    select
      w.client_key,
      nullif(trim(w.client_identifier_display), '') as ident
  ...
  work_inv as (
    select
      nullif(trim(i.company_code), '') as company_code,
      case
        when nullif(trim(i.vat_code), '') is not null
          and trim(i.vat_code) not ilike 'PERSON_%'
        then nullif(trim(i.vat_code), '')
        else null
      end as vat_code,
      nullif(trim(i.client_id), '') as client_id
    from active_work aw
    join public.invoices i
      on i.client_id = aw.client_key
      or coalesce(nullif(trim(i.company_code), ''), i.client_id, '') = aw.client_key
      or nullif(trim(i.company_code), '') = aw.client_key
      or (
        nullif(trim(i.vat_code), '') is not null
        and trim(i.vat_code) not ilike 'PERSON_%'
        and nullif(trim(i.vat_code), '') = aw.client_key
      )
      or (
        aw.ident is not null
        and (
          nullif(trim(i.company_code), '') = aw.ident
          or (
            nullif(trim(i.vat_code), '') is not null
            and trim(i.vat_code) not ilike 'PERSON_%'
            and nullif(trim(i.vat_code), '') = aw.ident
          )
          or i.client_id = aw.ident
        )
      )
    where i.invoice_number like 'VK-000%'
      and i.invoice_number not like 'VK-000IS%'
  ...
  blocked as (
    select distinct ck
    from (
      select aw.client_key as ck from active_work aw
      union all
      select aw.ident
      from active_work aw
      where aw.ident is not null
        and aw.ident not ilike 'PERSON_%'
      union all
      select wi.company_code
      from work_inv wi
      where wi.company_code is not null
        and wi.company_code not ilike 'PERSON_%'
      union all
      select wi.vat_code from work_inv wi where wi.vat_code is not null
      union all
      select wi.client_id from work_inv wi where wi.client_id is not null
    ) s
    where nullif(trim(ck), '') is not null
  ),
  matched as (
    select
      a.k as client_key,
...
-- Tas pats blocking ir fallback RPC (pilnas sąrašas).
