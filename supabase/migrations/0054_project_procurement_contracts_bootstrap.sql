-- Pilnas viešųjų pirkimų sutarčių modulis, jei 0051 nebuvo pritaikyta (lentelės nebuvo).
-- Saugu: CREATE IF NOT EXISTS, ALTER IF NOT EXISTS.
...
create table if not exists public.project_procurement_contracts (
  ...
  import_dedupe_key text not null,
  contract_uid text not null default '',
  ...
  constraint project_procurement_contracts_project_id_import_dedupe_key_key unique (project_id, import_dedupe_key)
);
