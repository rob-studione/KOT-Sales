-- Greitesnis blocking: be OR join ant invoices (naudoja client_id / company_code indeksus).
-- Blokuoja kandidatą, jei aktyviame darbe tas pats client_key / company_code / vat_code / client_id.
...
as $$
  ...
  -- Išplėsti work ID → company_code / vat_code per indeksuotus stulpelius (be OR join).
  work_business_ids as (
    select distinct nullif(trim(i.company_code), '') as id
    from public.invoices i
    where p_project_id is not null
    ...
    union
    select distinct nullif(trim(i.vat_code), '')
    from public.invoices i
    where p_project_id is not null
      and i.client_id in (select id from active_work_ids)
      and i.invoice_number like 'VK-000%'
    ...
    union
    select distinct nullif(trim(i.company_code), '')
    from public.invoices i
    where p_project_id is not null
      and nullif(trim(i.company_code), '') in (select id from active_work_ids)
      and i.invoice_number like 'VK-000%'
      and i.invoice_number not like 'VK-000IS%'
      and i.invoice_number not like 'VK-000KR%'
      and trim(i.company_code) not ilike 'PERSON_%'
    ...
    from public.invoices i
    where p_project_id is not null
      and nullif(trim(i.vat_code), '') in (select id from active_work_ids)
      and i.invoice_number like 'VK-000%'
      and i.invoice_number not like 'VK-000IS%'
      and i.invoice_number not like 'VK-000KR%'
      and trim(i.vat_code) not ilike 'PERSON_%'
    union
    select distinct nullif(trim(i.client_id), '')
    from public.invoices i
    where p_project_id is not null
      and (
        nullif(trim(i.company_code), '') in (select id from active_work_ids)
        or nullif(trim(i.vat_code), '') in (select id from active_work_ids)
      )
      and i.invoice_number like 'VK-000%'
      and i.invoice_number not like 'VK-000IS%'
      and i.invoice_number not like 'VK-000KR%'
      and nullif(trim(i.client_id), '') is not null
  ),
  blocked as (
    select id as ck from active_work_ids
    union
    select id from work_business_ids where id is not null
  ),
  matched as (
    select
      a.k as client_key,
