-- Filtrai rankinių kandidatų RPC: p_status (crm_status), p_search (company_name / company_code + linked per view).
-- Deterministinis ORDER BY (apyvarta → pavadinimas → kodas → id), kad LIMIT/OFFSET būtų stabilūs po masinio importo
