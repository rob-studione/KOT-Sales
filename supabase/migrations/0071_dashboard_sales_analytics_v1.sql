-- Dashboard (v1): KPI + trend + conversion + sales KPI vienu RPC.
-- Tikslas: nebetempti raw `project_work_item_activities` ir `invoices` eilučių į Node.
--
-- Semantika atitinka dabartinį app kodą (`lib/crm/salesAnalyticsDashboard.ts`):
-- - calls/answeredCalls/commercialActions/trend skaičiuojami iš VISŲ activity eilučių intervale (net jei nėra `client_key`)
-- - attribution (first_call/latest_status/conversion/sales KPI) skaičiuojamas tik tiems įrašams, kurie turi `client_key`
-- - answeredCalls: pagal Kanban normalizaciją ir `ANSWERED_STATUSES`
-- - actualu: regex "aktualu pagal poreikį" per `next_action` arba `call_status` (tame pačiame lange)
-- - invoice_date > first_call_day: lyginama su UTC diena iš `occurred_at` (kaip Node `slice(0,10)`)
-- - conversionPercent: distinct klientai su invoice (range lange) / calls (range lange)
-- - avgEurPerCall: directRevenueEur / calls (sales lange)
--
-- Pastaba (v1): invoices client_key mapping remiasi prielaida:
--   coalesce(nullif(trim(company_code), ''), client_id, '').
-- Jei `project_work_items.client_key` ne visur sutampa su šiuo formatu, tai v2 vieta tikslinti mapping'ą.
...
acts_flags as (
  select
    au.*,
    -- `normalizeKanbanCallStatus` (be enum dinaminių statusų)
    case
...
-- ------------------------------------------------------------
-- KPI: veikla (range) - iš visų eilučių (be client_key filtro)
-- ------------------------------------------------------------
kpi_activity as (
  select
...
-- ------------------------------------------------------------
-- Trend: visos dienos su nuliais (range) - iš visų eilučių
-- ------------------------------------------------------------
trend_days as (
...
-- ------------------------------------------------------------
-- Attribution helpers (range + sales) - tik kai yra client_key
-- ------------------------------------------------------------
first_call_range as (
  select
    client_key,
...
-- ------------------------------------------------------------
-- Invoices (union) su client_key mapping v1 (be view join)
-- ------------------------------------------------------------
invoices_union as (
  select
    i.invoice_id,
...
-- conversion: klientas su invoice range lange po pirmo skambučio (range)
...
-- sales KPI: direct/influenced pagal latest_status_sales (sales)
sales_kpi as (
  select
