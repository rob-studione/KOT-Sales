...
import { workItemActionTypeLabel, workItemActivityOutcomeLine } from "@/lib/crm/projectBoardConstants";
...
export function WorkItemActivityTimeline({
  ...
  return (
    <ul className={compact ? "space-y-2" : "max-h-[40vh] space-y-3 overflow-y-auto pr-1"}>
      {list.map((a) => {
        const outcome = workItemActivityOutcomeLine(a.action_type, a.call_status);
        return (
        <li
          key={a.id}
        ...
        >
          ...
          <div className="mt-1 space-y-0.5 text-xs text-zinc-600">
            <div>
              <span className="text-zinc-400">{outcome.label}: </span>
              {outcome.value}
            </div>
            {a.next_action_date ? (
              <div>
