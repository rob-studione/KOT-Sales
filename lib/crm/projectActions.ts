...
export async function markCandidateAsInvalidAction(
  candidateId: string
): Promise<{ ok: true } | { ok: false; error: string }> {
  const id = String(candidateId ?? "").trim();
  if (!id || !isValidUuid(id)) return { ok: false, error: "Neteisingas kandidato identifikatorius." };
  ...
  if (error) {
    console.error("[projectActions] markCandidateAsInvalidAction failed", error);
...
export async function restoreCandidateAction(
  ...
  if (error) {
    console.error("[projectActions] restoreCandidateAction failed", error);
...
export async function markAutoCandidateAsInvalidAction(
  ...
  const { error } = await supabase
    .from("project_candidate_statuses")
    .upsert(
      { project_id: pid, client_key: ck, status: "netinkamas", updated_at: new Date().toISOString() },
      { onConflict: "project_id,client_key" }
    );
...
export async function restoreAutoCandidateAction(
  ...
  const { error } = await supabase
    .from("project_candidate_statuses")
    .upsert(
      { project_id: pid, client_key: ck, status: "active", updated_at: new Date().toISOString() },
      { onConflict: "project_id,client_key" }
    );
